...
namespace Game {
    ...
    namespace Chunks {
        ...
        {
            const c = new Uint8Array(Math.pow(size, 3))
            chunks.push(c);
            c.fill(13)
            return c;
        }
    ...
    namespace Assets {
        ...
        export const camera = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight)
    ...
    namespace Player {
        ...
        export function create(): InnerPlayer
        {
    ...
    let chunkMesh = new THREE.Mesh(
        new THREE.BufferGeometry(),
        new THREE.MeshBasicMaterial({map, wireframe: true, vertexColors: false, transparent: true})
    )

    const chunk = Chunks.generate();


    function getBlock(chunk: Uint8Array, x: number, y: number, z: number){
